...
Page({
  ...
  onCancelOrder: function (e) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          Trade.del({ id: id }).then(function (res) {
  ...
  loadTradesFromRemote: function (options) {
    Trade.getTrades_buyer(options).then(function (trades) {
      ...
      let _trades = []
      for (let i in trades) {
        if (trades[i].status == options.status) {
  ...
  loadTrades: function (options = {}) {
    ...
    if (status == '未提交') {
      ...
      this.loadTradesFromRemote(options)
    }
  },
